             ...
             void       *cbArg)  	// I - Callback data
{
    ...
    // comment character...
    start = editwindow->m_buffer->line_start(pos);
//  if (start > 0) start = editwindow->m_buffer->line_start(start - 1);
    end   = editwindow->m_buffer->line_end(pos + nInserted);
    text  = editwindow->m_buffer->text_range(start, end);
...
//    printf("Recalculate the rest of the buffer style\n");
        ...
        end   = editwindow->m_buffer->length();
        text  = editwindow->m_buffer->text_range(start, end);
        style = stylebuf->text_range(start, end);
...
style_init(void)
{
    char *style = new char[editwindow->m_buffer->length() + 1];
    char *text = editwindow->m_buffer->text();

    memset(style, 'A', editwindow->m_buffer->length());
    style[editwindow->m_buffer->length()] = '\0';

    if (!stylebuf) stylebuf = new Fl_Text_Buffer(editwindow->m_buffer->length());

    style_parse(text, style, editwindow->m_buffer->length());
